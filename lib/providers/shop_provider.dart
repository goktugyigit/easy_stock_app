...
class ShopProvider with ChangeNotifier {
  ...
  Future<void> fetchAndSetItems({bool forceFetch = false}) async {
    if (_isFetching || (!forceFetch && _hasFetchedOnce)) {
      debugPrint(
          "ShopProvider: fetchAndSetItems çağrısı engellendi. isFetching: $_isFetching, hasFetchedOnce: $_hasFetchedOnce, forceFetch: $forceFetch");
    ...
    debugPrint(
        "ShopProvider: fetchAndSetItems BAŞLADI. forceFetch: $forceFetch");
    ...
    try {
      ...
      if (!prefs.containsKey(_storageKey)) {
        debugPrint(
            "ShopProvider: SharedPreferences'te anahtar yok (_storageKey: $_storageKey).");
      } else {
        ...
        if (extractedData == null || extractedData.isEmpty) {
          ...
          loadedItems = extractedData
              .map((itemJson) {
                try {
                  return ShopItem.fromMap(
                      json.decode(itemJson) as Map<String, dynamic>);
                } catch (e) {
                  debugPrint(
                      'Bir dükkan parse edilirken hata (fetchAndSetItems): $e, JSON: $itemJson');
                  return null;
                }
              })
              ...
              .toList();
          debugPrint(
              "ShopProvider: SharedPreferences'ten ${loadedItems.length} dükkan yüklendi.");
        }
      }
      ...
      debugPrint(
          "ShopProvider: fetchAndSetItems BİTTİ ve notifyListeners çağrıldı. Items count: ${_items.length}");
    } catch (error) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        notifyListeners();
      }); // Hata durumunda da UI güncellensin (boş liste ile)
    } finally {
      _isFetching = false;
    }
  }
  ...
  Future<void> _saveItemsToPrefs() async {
    try {
      ...
      debugPrint(
          "ShopProvider: ${_items.length} dükkan SharedPreferences'e kaydedildi.");
    } catch (error) {
  ...
  void addItem({
    ...
    debugPrint(
        "ShopProvider: addItem BAŞLADI - Mevcut items: ${_items.length}");
