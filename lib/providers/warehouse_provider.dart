...
class WarehouseProvider with ChangeNotifier {
  ...
  Future<void> fetchAndSetItems({bool forceFetch = false}) async {
    if (_isFetching || (!forceFetch && _hasFetchedOnce)) {
      debugPrint(
          "WarehouseProvider: fetchAndSetItems çağrısı engellendi. isFetching: $_isFetching, hasFetchedOnce: $_hasFetchedOnce, forceFetch: $forceFetch");
      return;
    }
    _isFetching = true;
    debugPrint(
        "WarehouseProvider: fetchAndSetItems BAŞLADI. forceFetch: $forceFetch");
    ...
    try {
      ...
      if (!prefs.containsKey(_storageKey)) {
        debugPrint(
            "WarehouseProvider: SharedPreferences'te anahtar yok (_storageKey: $_storageKey).");
      } else {
          ...
          loadedItems = extractedData
              .map((itemJson) {
                try {
                  ...
                  debugPrint(
                      'Bir depo parse edilirken hata (fetchAndSetItems): $e, JSON: $itemJson');
                  return null;
                }
              })
              ...
              .toList();
          debugPrint(
              "WarehouseProvider: SharedPreferences'ten ${loadedItems.length} depo yüklendi.");
        }
      }
      _items = loadedItems;
      _hasFetchedOnce = true;
      // Widget tree inşa edilirken notifyListeners çağrılmasını engelle
      WidgetsBinding.instance.addPostFrameCallback((_) {
        notifyListeners();
      });
      debugPrint(
          "WarehouseProvider: fetchAndSetItems BİTTİ ve notifyListeners çağrıldı. Items count: ${_items.length}");
    } catch (error) {
      debugPrint(
          "WarehouseProvider: fetchAndSetItems sırasında genel hata: $error");
      _items = [];
      _hasFetchedOnce = true;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        notifyListeners();
  ...
  Future<void> _saveItemsToPrefs() async {
    try {
      ...
      debugPrint(
          "WarehouseProvider: ${_items.length} depo SharedPreferences'e kaydedildi.");
    } catch (error) {
      if (kDebugMode) {
  ...
  void addItem({
    ...
    debugPrint(
        "WarehouseProvider: addItem BAŞLADI - Mevcut items: ${_items.length}");
    ...
    if (kDebugMode) {
      print(
          'Depo Eklendi (Provider): ${newItem.name}, Yeni Toplam: ${_items.length}');
    }
    notifyListeners();
