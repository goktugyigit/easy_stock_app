...
class _AddEditWarehousePageState extends State<AddEditWarehousePage> {
  ...
  var _editedWarehouseItem = WarehouseItem(id: '', name: ''); // Varsayılan değerler
  var _isInit = true;
  ...
  void didChangeDependencies() {
    if (_isInit) {
      if (widget.existingItemId != null) {
        final existingItem = Provider.of<WarehouseProvider>(context, listen: false)
  ...
  Future<void> _saveForm() async {
    ...
    _formKey.currentState!.save(); // onSaved'ları tetikler (şu an kullanmıyoruz ama iyi pratik)
      ...
      if (mounted) {
        await showDialog<void>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text('Depo kaydedilirken bir sorun oluştu: ${error.toString()}'),
    ...
    if (_pickedImageFile == null) {
      return Column(
        ...
        children: [
          ...
          Text('Depo Resmi Seç', style: TextStyle(color: Colors.grey[800], fontSize: 13)),
        ],
      );
    }
    if (kIsWeb) {
                ...
                child: ListView(
                  children: <Widget>[
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(labelText: 'Depo Adı (*)', border: OutlineInputBorder()),
                      textInputAction: TextInputAction.next,
                      validator: (v) => (v == null || v.isEmpty) ? 'Depo adı zorunludur.' : null,
                    ),
                    ...
                    ElevatedButton.icon(
                      icon: const Icon(Icons.save_alt_outlined),
                      label: const Text('Depoyu Kaydet'),
                      onPressed: _isLoading ? null : _saveForm,
