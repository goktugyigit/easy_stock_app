...
class FanusPainterConfig {
  ...
  Color getRingColor(double levelPercentage) {
    if (levelPercentage <= 0.01) return Colors.grey.shade500;
  ...
  // DÜZELTME: static final defaultConfig'i const yapıyoruz.
  static const FanusPainterConfig defaultConfig = FanusPainterConfig(
    layers: [ // Bu listenin de const olması gerekiyor.
...
class _FanusWidgetState extends State<FanusWidget> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(covariant FanusWidget oldWidget) {
    ...
    if (oldWidget.currentStock != widget.currentStock || oldWidget.maxStock != widget.maxStock) {
      _updateTargetLevel();
    }
  }
  ...
  void _updateTargetLevel() {
    ...
    if (widget.maxStock > 0) {
      targetPercentage = (widget.currentStock / widget.maxStock) * 100.0;
    } else if (widget.currentStock > 0) {
      targetPercentage = 100.0;
    }
    _targetLevelPercentage = targetPercentage.clamp(0.0, 100.0);
  }
    ...
    return SizedBox(
      ...
      child: CustomPaint(
        painter: _FanusPainter(
          ...
          actualStockText: widget.currentStock.toString(),
          animationTime: _animationTime,
          config: FanusPainterConfig.defaultConfig, // Bu zaten const
        ),
      ),
    );
  }
...
class _FanusPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final baseY = displayLevelPercentage <= 0.001
        ? size.width + maxAmp
        : math.min(size.width - bottomClearance, math.max(topClearance, rawY));

    if (displayLevelPercentage > .001) {
    ...
    final textOffset = Offset(r - textPainterStroke.width / 2, r - textPainterStroke.height / 2);
