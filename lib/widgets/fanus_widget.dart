...
class FanusPainterConfig {
  ...
  Color getRingColor(double levelPercentage) {
    ...
    return HSLColor.fromAHSL(
            1.0, h.clamp(0, 360), 0.80, l.clamp(0, 100) / 100.0)
        .toColor();
  }
...
class _FanusWidgetState extends State<FanusWidget>
    ...
    _ticker = createTicker((_) {
      if (mounted) {
        setState(() {
          ...
          _displayLevelPercentage +=
              (_targetLevelPercentage - _displayLevelPercentage) * _easeFactor;
        });
      }
    })
      ..start();
...
class _FanusPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final clip = Path()
      ..addOval(Rect.fromCircle(center: centerOffset, radius: innerR));
    canvas.save();
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = radialGradient.createShader(
            Rect.fromCircle(center: Offset(r * 0.7, r * 0.7), radius: innerR)),
    );
    canvas.drawColor(
        Colors.white.withAlpha((0.6 * 255).round()), BlendMode.srcOver);
    ...
    if (displayLevelPercentage > 1) {
      ...
      for (var i = 0; i < config.layers.length; i++) {
        ...
        final amp = layer.amp *
            (1 -
                config.jitterAmp / 2 +
                config.jitterAmp * 0.5 * (1 + math.sin(t + layer.phase)));
        final len = layer.len *
            (1 -
                config.jitterLen / 2 +
                config.jitterLen * 0.5 * (1 + math.cos(t * 0.7 - layer.phase)));
        ...
        canvas.drawPath(
          ...
          Paint()
            ..shader = LinearGradient(
              ...
              Rect.fromLTWH(0, baseY - amp * 1.5, size.width,
                  (centerOffset.dy + innerR) - (baseY - amp * 1.5)),
            ),
        );
      }
    }
    ...
    canvas
      ...
      ..drawArc(
        arcRect,
        -math.pi / 2,
        (config.clockwiseFill ? 1 : -1) *
            2 *
            math.pi *
            displayLevelPercentage /
            100,
        false,
        fillRingPaint,
      );
    ...
    if (unitText == null || unitText!.isEmpty) {
      ...
      final textPainter = TextPainter(
          text: TextSpan(text: actualStockText, style: textStyleFill),
          textAlign: TextAlign.center,
          textDirection: TextDirection.ltr)
        ..layout(maxWidth: size.width);
      final textPainterStroke = TextPainter(
          text: TextSpan(text: actualStockText, style: textStyleStroke),
          textAlign: TextAlign.center,
          textDirection: TextDirection.ltr)
        ..layout(maxWidth: size.width);
      final textOffset =
      ...
      // Eğer birim belirtilmişse, miktar üstte, birim altta küçük olur
      // Miktar için stiller
      ...
      final valueTextStyleFill = TextStyle(
          color: ringColorValue,
          fontWeight: FontWeight.w700,
          fontSize: size.width * .25,
          fontFamily: 'Poppins');
      ...
      final unitTextStyle = TextStyle(
          color: const Color.fromARGB(255, 36, 1, 1),
      ...
      final valuePainter = TextPainter(
          text: TextSpan(text: actualStockText, style: valueTextStyleFill),
          textAlign: TextAlign.center,
          textDirection: TextDirection.ltr)
        ..layout(maxWidth: size.width);
      final valuePainterStroke = TextPainter(
          text: TextSpan(text: actualStockText, style: valueTextStyleStroke),
          textAlign: TextAlign.center,
          textDirection: TextDirection.ltr)
        ..layout(maxWidth: size.width);
      ...
      final unitPainter = TextPainter(
          text: TextSpan(text: unitText, style: unitTextStyle),
          textAlign: TextAlign.center,
          textDirection: TextDirection.ltr)
        ..layout(maxWidth: size.width);
      ...
      final valueOffset =
          Offset(r - valuePainter.width / 2, r - totalHeight / 2);
      final unitOffset = Offset(r - unitPainter.width / 2,
          valueOffset.dy + valuePainter.height * 0.9);
